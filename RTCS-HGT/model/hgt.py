...
class HGTLayer(nn.Module):
    ...
    def forward(self, G, inp_key, out_key):
        ...
        G.multi_update_all({etype: (self.message_func, self.reduce_func) for etype in edge_dict}, cross_reducer="mean")
    ...
    def __repr__(self):
        return "{}(in_dim={}, out_dim={}, num_types={}, num_types={})".format(self.__class__.__name__, self.in_dim, self.out_dim, self.num_types, self.num_relations)


class HGT(nn.Module):
    def __init__(self, G, n_inp_dict, n_hid, n_out, n_layers, n_heads, dropout=0.2, use_norm=False):
        ...
        for ntype in G.ntypes:
            self.adapt_ws.append(nn.Linear(n_inp_dict[ntype], n_hid))
        for _ in range(n_layers):
            self.gcs.append(HGTLayer(n_hid, n_hid, len(G.node_dict), len(G.edge_dict), n_heads, dropout=dropout, use_norm=use_norm))
    ...
    def forward(self, G, out_key):
        for ntype in G.ntypes:
            try:
                ...
                raise KeyError(f"KeyError: 'inp'; ntype: {ntype}")
        for i in range(self.n_layers):
            self.gcs[i](G, "h", "h")
        return self.out(G.nodes[out_key].data["h"]), G.nodes["user"].data["h"]

    def __repr__(self):
        return "{}(n_inp_dict={}, n_hid={}, n_out={}, n_layers={})".format(self.__class__.__name__, self.n_inp, self.n_hid, self.n_out, self.n_layers)
